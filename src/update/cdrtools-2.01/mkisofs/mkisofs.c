...
/* NLS-CJK_FIX Japan NOBORU noboru2000@yahoo.co.jp 27.12.2004 */

/* NLS-2-MD5 Wwashington http://code.google.com/p/mkisofs-md5 */
...
char	version_string[] = "GNU Released by Joerg Schilling, Modified by Wwashington. With Great\nThanks To: Japan NOBORU(NLS-CJK Patch), Alex Kopylov(MD5-Opt Patch).\nmkisofs 2.01-bootcd.ru-cjk-md5";
...
int	cjk4dos_filenames = 0;	/* For GbChs.  Disc possible work with DOS */
int	cjk4unx_filenames = 0;	/* For GbChs.  Disc possible work with DOS */
...
#define OPTION_CJK4DOS_FILENAMES	1098
#define OPTION_CJK4UNX_FILENAMES	1099
...
/* -----> END POSIX.1-2001 REMOVE */
	...
	{{"cjk4dos-filenames", no_argument, NULL, OPTION_CJK4DOS_FILENAMES},
	'\0', NULL, "Allow 8 bit ASCII CJK standard code characters (violates ISO9660)", ONE_DASH},
	{{"cjk4unx-filenames", no_argument, NULL, OPTION_CJK4UNX_FILENAMES},
	'\0', NULL, "Allow 8 bit ASCII CJK standard code characters (violates ISO9660)", ONE_DASH},
 ...
 */
		...
		case OPTION_RELAXED_FILENAMES:
			...
			warn_violate++;
			break;
		case OPTION_CJK4DOS_FILENAMES:
			cjk4dos_filenames++;
			warn_violate++;
			break;
		case OPTION_CJK4UNX_FILENAMES:
			cjk4unx_filenames++;
			warn_violate++;
			break;
...
parse_input_files:
	...
	if (in_nls == NULL || out_nls == NULL) { /* Unknown charset specified */
		...
		if ((nl = list_nls()) != NULL) {
			fputs(nl, stderr); /* List all known charset names */
			free(nl);
		}
		exit(1);
	}
	...
	if (hfs_inls == NULL || hfs_onls == NULL) {
		char  *nl;
		fprintf(stderr, "hfs_icharset = %s\n", hfs_icharset ? hfs_icharset : "(null)");
		fprintf(stderr, "hfs_inls = %p\n", hfs_inls);
		fprintf(stderr, "hfs_ocharset = %s\n", hfs_ocharset ? hfs_ocharset : "(null)");
		fprintf(stderr, "hfs_onls = %p\n", hfs_onls);
